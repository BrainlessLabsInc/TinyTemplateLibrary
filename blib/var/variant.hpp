...
#include "blib/meta/is_pointer.hpp"
...
{
	exception() : blib::exception("variant error") {}
};
...
namespace impl
{
	...
	struct copier
	{
		...
		void operator()()
		{
			...
			const blib::data_holder<T>* rd = static_cast<const blib::data_holder<T>*>(r_.pnt_);
			l_.destroy();
			l_.pnt_ = new(l_.stor_.buf_) blib::data_holder<T>(rd->d);
			l_.which_ = r_.which_;
		};
	};
...
///////////////////////////////////////////////////////////////////////	
	...
	struct variant
	{
		...
		enum
		{
			size = sizeof(blib::data_holder<typename list::largest_type>)
		};
		...
		variant( const T& r ) : which_(0), pnt_(0)
		{
			...
			pnt_ = new(stor_.buf_) blib::data_holder<typename found::type>(r);
			which_ = found::index;
		}
		...
		template< typename T >
		const blib::data_holder<T>* get_holder() const
		{ 
			return static_cast<const blib::data_holder<T>*>(pnt_);
		}

		template< typename T >
		blib::data_holder<T>* get_holder()
		{ 
			return static_cast<blib::data_holder<T>*>(pnt_);
		}
	...
	private:
		...
		struct storage
		{
			union
			{
				blib::data_holder_base dummy;  //hope to satisfy alignment settings
				char buf_[N];
			};
		};
		
		int which_;
		blib::data_holder_base* pnt_;
		storage< sizeof(blib::data_holder<typename list::largest_type>) > stor_;
		
		struct deleter
		{
			blib::data_holder_base *p_;
			deleter( blib::data_holder_base *p ) : p_(p) {}
			
			template<typename T>
			void operator()() 
			{
				typedef blib::data_holder<T> var_type;
	...
	< 
		...
		typename blib::data_holder<T>::const_pointer,
		typename blib::data_holder<T>::pointer
	>::type 
	get( V* v )
	{
	...
	typename selector
		< 
			...
			typename blib::data_holder<T>::const_return_type,
			typename blib::data_holder<T>::return_type
		>::type
	get( V& v )
	{
		typedef meta::find_equivalent_type<T, typename V::list> found;
